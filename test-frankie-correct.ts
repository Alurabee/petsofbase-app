...
// EXACT prompts from server/imageGeneration.ts
const stylePrompts = {
...
const styles = [
  { name: "Pixar", prompt: `A cute brown dachshund dog. ${stylePrompts.pixar}` },
  { name: "Cartoon", prompt: `A cute brown dachshund dog. ${stylePrompts.cartoon}` },
  { name: "Realistic", prompt: `A cute brown dachshund dog. ${stylePrompts.realistic}` },
  { name: "Anime", prompt: `A cute brown dachshund dog. ${stylePrompts.anime}` },
  { name: "Watercolor", prompt: `A cute brown dachshund dog. ${stylePrompts.watercolor}` }
];

async function testFrankieCorrect() {
  console.log('🐕 Regenerating Frankie with CORRECT prompts from app...\n');
  ...
  for (const style of styles) {
    ...
    try {
      ...
      // Save the result
      if (result.url) {
        const outputPath = path.join(__dirname, `frankie-v2-${style.name.toLowerCase()}.png`);
  ...
  console.log('='.repeat(50));
  console.log('\n✨ All styles regenerated! Check frankie-v2-*.png files.');
}
